...
} from "../../middleware/middleware.validate";
...
export class questionRoute {
  ...
  public route(app: Application) {
    ...
    app
      .route("/v1/question/test")
      .all(isAuthen)
      .get(this.quest.createTest)
      .post(validateBodySchema(categorySchema), this.quest.createTestCategory);

    app
      .route("/v1/manager/:english")
      .all(isAuthen)
      .put(
        isAdmin,
        validateBodySchema(updateQuestionSchema),
        this.quest.updateQuestion
      )
      .delete(
        isAdmin,
        validateParamsSchema(englishSchema),
        this.quest.deleteQuestion
      )
      .get(this.quest.getOne);

    app.route("/v1/question/all").get(isAuthen, this.quest.getAll);

    app.route("/v1/category/all").get(isAuthen, this.quest.getCategory);
  }
}
