  ...
  async crawlData(page: Number): Promise<any> {
    ...
    $(".noidung").each((i, element) => {
      ...
      vietnam = vietnam
        .text()
        .trim()
        .replace(vietnam.children().text().trim(), "")
        .trim();
      const result = {
  ...
  async createMultipleChoice(data: any, answer: any): Promise<any> {
    try {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        while (
          temp.includes(randomAnswer) ||
          data.english == answer[randomAnswer].english
        ) {
